...
describe('EngineClient', () => {
  ...
  describe('constructor', () => {
    it('strips trailing slashes from baseUrl', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(textResponse('ok'));
      await client.health();
  ...
  describe('health()', () => {
    ...
    it('returns error on non-ok response', async () => {
      vi.spyOn(globalThis, 'fetch').mockResolvedValue(textResponse('Service Unavailable', 503));
      const client = new EngineClient('http://localhost:4000');
      const result = await client.health();
  ...
  describe('createStatechart()', () => {
    const snapshot = {
      ...
      active_states: [{ id: 's1', status: 'running' as const, type: 'atomic' as const }],
    };

    it('sends document as string', async () => {
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(jsonResponse(snapshot, 201));
      const client = new EngineClient('http://localhost:4000');
      const result = await client.createStatechart(JSON.stringify({ scxml: {} }));
  ...
  describe('sendEvent()', () => {
    const snapshot = {
      ...
      execution_status: 'idle' as const,
      active_states: [{ id: 's2', status: 'running' as const, type: 'atomic' as const }],
    };
